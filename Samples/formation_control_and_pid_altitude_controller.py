...
class ShapeController:
    """Simple Airsim Implementation of a shape_controller with altitude 
    control """
    ...
    def has_reached_flight_operation_altitude(self):
        tolerance = 0.2
        for altitude in self.get_drone_altitude():
            print("Altitude of a drone:", altitude)
            # we still need to climb
            if abs(altitude) < abs(self.flight_operation_altitude - tolerance):
                return False
        # reached flight altitude
        return True
    ...
    def drone_motion_control(self, tau_dot, altitude_control_input):
        ...
        for index, drone_name in enumerate(self.drone_names_list):
            ...
            zv = altitude_control_input[index]
            print("command velocity vector: ", xv, yv, zv)
    ...
    #    if not self.has_reached_flight_operation_altitude():
    #        self.drone_motion_control()
    ...
    def shape_controller(self):
        ...
        while True: # self.has_converged()
            ...
            # Motion Control
            self.drone_motion_control(control_signal, control_signal_altitude)
