...
class QuadEnergyModel:
    ...
    def get_drone_velocity(self, drone):
        ...
        kinematics = self.airsim_client.simGetGroundTruthKinematics(drone)
        velocity_drone = [kinematics.linear_velocity.x_val, kinematics.linear_velocity.y_val, kinematics.linear_velocity.z_val]
        return np.array(velocity_drone) # veirfy with Alain
    

    def get_velocity_air(self, drone, wind_vector):
        # Velocity_Air = ||V_air|| = || V_ground (velocity drone is travelling) - V_wind ||
        velocity_ground = self.get_drone_velocity(drone)
        velocity_air = np.linalg.norm(velocity_ground - wind_vector)
        return velocity_air


    def get_net_thrust(self, drone):
        # computes net thrust of all 4 props
        rotor_states = self.airsim_client.getRotorStates(drone)
        net_thrust = 0
    ...
    def get_thrust(self, drone):
        # returns thrust of all four props in an array
        rotor_states = self.airsim_client.getRotorStates(drone)
    ...
    def get_angular_speed(self, drone):
        # returns speed of all four props in an array
        rotor_states = self.airsim_client.getRotorStates(drone)
        speed = []
        for rotor in rotor_states.rotors:
    ...
    def get_velocity_induced(self, drone):
        ...
        velocity_induced = sqrt( thrust / (2 * air_density * A) )
    ...
    def induced_power(self, drone):
        ...
        thrust = self.get_net_thrust(drone)
        induced_velocity = self.get_velocity_induced(drone)
        induced_power = thrust * induced_velocity
    ...
    def profile_power(self, drone):
        ...
        # R: radius of propellers
        total_rotors = 4
        N = 2
        c = 0.10 # ask alain for accurate value from cad model
        cd_blade = 1.28 # verify with Alain, currently selected a random approximation
        R = 0.12 # meters verify with Alain
        air_density = 1.204 # kg/m^3
        angular_speeds = self.get_angular_speed(drone)
        profile_power = 0
        for ang_speed in angular_speeds:
            profile_power += ( ( N * c * cd_blade * air_density * pow(R, 4) ) / ( 8 * pow(ang_speed, 3) ) )
        return profile_power
    
    
    def parasitic_power(self, drone):
        # Formula: F_Drag * Air_velocity = 0.5 * Coef_drag * air_density * A_ref * Air_velocity^3
        wind_vector = np.array([0,0,0])
        velocity_air = self.get_velocity_air(drone, wind_vector)
        coef_drag = 1.139 # drag coefficient must ask alain about this
        air_density = 1.204 # kg/m^3
        A_ref = 0 # ask alain about what this area is
        parasite_power = 0.5 * coef_drag * air_density * A_ref * pow(velocity_air, 3)
        return parasite_power
        
    
    def power_model(self, drone):
        """ Compute Power Usage in hover state """        
        total_power = self.induced_power(drone) + self.profile_power(drone) + self.parasitic_power(drone)
        print(total_power)
        return total_power
    ...
    def drone_data_collection(self):
        ...
        drone_pose_WFNED = self.airsim_client.simGetObjectPose(drone_name)
        power_usage = self.power_model(drone_name)
        ...
        print("**********************************************************")
        print("Dron Energy Usage in Watts", power_usage, "\n")
        print("**********************************************************")
